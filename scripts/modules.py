...
class TransformerFUNet(nn.Module):
    def __init__(self, cfg: LOBConfig):
        ...
        self.register_buffer("rope_freqs", compute_rope_freqs(self.levels, head_dim), persistent=False)
    ...
    def _get_rope_freqs(self, device: torch.device) -> torch.Tensor:
        if self.rope_freqs.device != device or self.rope_freqs.shape[0] != self.levels:
            head_dim = self.in_proj.out_features // self.n_heads
            self.rope_freqs = compute_rope_freqs(self.levels, head_dim, device=device)
        return self.rope_freqs

    def forward(self, x: torch.Tensor, ctx_tokens: torch.Tensor, adaln_cond: torch.Tensor) -> torch.Tensor:
        bsz = x.shape[0]
        tokens = x.view(bsz, self.levels, self.token_dim)
