...
class LSTMContextEncoder(nn.Module):
    def __init__(self, cfg: LOBConfig):
        super().__init__()
        self.rnn = nn.LSTM(cfg.state_dim, cfg.model.hidden_dim, num_layers=1, batch_first=True)
...
class TransformerContextEncoder(nn.Module):
    def __init__(self, cfg: LOBConfig):
        super().__init__()
        hidden_dim = cfg.model.hidden_dim
        self.in_proj = nn.Linear(cfg.state_dim, hidden_dim)
...
class MultiScaleContextEncoder(nn.Module):
    def __init__(self, cfg: LOBConfig):
        ...
        self.scales = (1, 5, 10)
        self.pool_projs = nn.ModuleList([nn.Linear(cfg.state_dim, cfg.model.hidden_dim) for _ in self.scales[1:]])
...
class SharedConditioningBackbone(nn.Module):
    def __init__(self, cfg: LOBConfig):
        ...
        self.x_proj = nn.Linear(cfg.state_dim, cfg.model.hidden_dim)
    ...
    def build_conditioning(
        ...
        ctx_tokens, _ = self.context_encoder(hist)
        ...
        ctx = self.cross(query, ctx_tokens)
        return ConditioningState(ctx=ctx, t_emb=t_emb, cond_emb=cond_emb, ctx_tokens=ctx_tokens)
