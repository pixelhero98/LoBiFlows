#!/usr/bin/env python3
"""Compare Euler vs DPM++-style sampling for the current LoBiFlow presets."""
...
from lobiflow.trainers.lob_train_val import eval_many_windows, save_json, seed_all, train_loop
...
def _evaluate_solver(ds_eval, model, cfg, *, horizons: Sequence[int], nfe: int, n_windows: int, seed_base: int) -> Dict[str, Any]:
...
def main():
    ...
    for dataset in datasets:
        ...
        solver_seed_runs: Dict[str, List[Dict[str, Any]]] = {"euler": [], "dpmpp2m": []}
        for seed in seeds:
            print(f"[solver-variants] dataset={dataset} seed={seed}")
            seed_all(int(seed))
            t0 = time.time()
            model = train_loop(
                ds_train,
            ...
            for idx, solver_name in enumerate(("euler", "dpmpp2m")):
                model.cfg.apply_overrides(solver=solver_name)
                cfg.apply_overrides(solver=solver_name)
                results = _evaluate_solver(
                    ds_test,
                    model,
                    cfg,
                    ...
                    seed_base=int(seed + idx * 100_000),
                )
                payload = {
                    "seed": int(seed),
                    "solver": solver_name,
                    ...
                    "results": results,
                    "macro": _macro_metrics(results, plan.horizons),
                }
                solver_seed_runs[solver_name].append(payload)

            save_json(
                {
                    "dataset": dataset,
                    "seed": int(seed),
                    "euler": solver_seed_runs["euler"][-1],
                    "dpmpp2m": solver_seed_runs["dpmpp2m"][-1],
                },
        ...
        for solver_name, runs in solver_seed_runs.items():
            dataset_summary["solvers"][solver_name] = {
                "seed_runs": runs,
                "aggregate": {
        ...
        e_score = dataset_summary["solvers"]["euler"]["aggregate"]["score_main"]["mean"]
        d_score = dataset_summary["solvers"]["dpmpp2m"]["aggregate"]["score_main"]["mean"]
        e_eff = dataset_summary["solvers"]["euler"]["aggregate"]["efficiency_ms_per_sample"]["mean"]
        d_eff = dataset_summary["solvers"]["dpmpp2m"]["aggregate"]["efficiency_ms_per_sample"]["mean"]
        dataset_summary["comparison"] = {
            "score_delta_dpmpp2m_minus_euler": float(d_score - e_score),
            "speedup_euler_to_dpmpp2m": float(e_eff / max(d_eff, 1e-12)),
        }
