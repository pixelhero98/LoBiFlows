...
DATASET_CHOICES = ("synthetic", "npz_l2", "optiver", "cryptos", "es_mbp_10")

LOBIFLOW_QUALITY_PRESETS: Mapping[str, Dict[str, object]] = {
    "synthetic": {
        "levels": 10,
        "history_len": 128,
        "eval_nfe": 2,
...
LOBIFLOW_SPEED_PRESETS: Mapping[str, Dict[str, object]] = {
    dataset: {
        **copy.deepcopy(dict(preset)),
        "eval_nfe": 1,
    }
    for dataset, preset in LOBIFLOW_QUALITY_PRESETS.items()
}

LOBIFLOW_PRESET_VARIANTS: Mapping[str, Mapping[str, Dict[str, object]]] = {
    "quality": LOBIFLOW_QUALITY_PRESETS,
    "speed": LOBIFLOW_SPEED_PRESETS,
}
...
def get_lobiflow_dataset_preset(dataset: str, variant: str = "quality") -> Dict[str, object]:
    variant_key = str(variant).strip().lower()
    if variant_key not in LOBIFLOW_PRESET_VARIANTS:
        raise ValueError(f"Unknown lobiflow preset variant={variant!r}")
    dataset_key = str(dataset).strip()
    presets = LOBIFLOW_PRESET_VARIANTS[variant_key]
    if dataset_key not in presets:
        raise ValueError(f"No LoBiFlow preset defined for dataset={dataset!r}")
    return copy.deepcopy(dict(presets[dataset_key]))


def apply_lobiflow_dataset_preset(args, variant: str | None = None):
    dataset = getattr(args, "dataset", None)
    if dataset not in LOBIFLOW_QUALITY_PRESETS:
        return args
    variant_name = str(variant or getattr(args, "lobiflow_variant", "quality")).strip().lower()
    preset = get_lobiflow_dataset_preset(str(dataset), variant=variant_name)
    for key, value in preset.items():
        if not hasattr(args, key):
            continue
        if getattr(args, key) is None:
            setattr(args, key, value)
    if hasattr(args, "lobiflow_variant") and getattr(args, "lobiflow_variant") is None:
        setattr(args, "lobiflow_variant", variant_name)
    return args
...
__all__ = [
    "DATASET_CHOICES",
    "DEFAULT_SYNTHETIC_LENGTH",
    "LOBIFLOW_QUALITY_PRESETS",
    "LOBIFLOW_SPEED_PRESETS",
    "mkdir",
    "parse_int_list",
    "parse_float_list",
