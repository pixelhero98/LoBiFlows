...
def test_imports():
    import lobiflow.trainers.benchmark_lobiflow_paper_ready as benchmark_lobiflow_paper_ready
    import lobiflow.trainers.benchmark_lobiflow_solver_variants as benchmark_lobiflow_solver_variants
    import lobiflow.trainers.benchmark_lobiflow_suite as benchmark_lobiflow_suite
    import lobiflow.trainers.benchmark_strict_baselines as benchmark_strict_baselines
    import lobiflow.trainers.context_architecture_followup as context_architecture_followup
    import lobiflow.models.deepmarket_baselines as deepmarket_baselines
    import lobiflow.trainers.experiment_common as experiment_common
    import lobiflow.utils.export_model_metric_catalogs as export_model_metric_catalogs
    import lobiflow.datasets.fit_lobster_synth_profile as fit_lobster_synth_profile
    import lobiflow.models.lob_baselines as lob_baselines
    import lobiflow.datasets.lob_datasets as lob_datasets
    import lobiflow.models.lob_model as lob_model
    import lobiflow.trainers.lob_train_val as lob_train_val
    import lobiflow.utils.lob_utils as lob_utils
    import lobiflow.utils.lob_viz_results as lob_viz_results
    import lobiflow.trainers.experiments_lobiflow as experiments_lobiflow
    import lobiflow.datasets.prepare_cryptos as prepare_cryptos
    import lobiflow.datasets.prepare_databento as prepare_databento
    import lobiflow.models.temporal_baselines as temporal_baselines

def test_config():
    from lobiflow.models.lob_baselines import LOBConfig, TransformerFUNet
    cfg = LOBConfig()
...
def test_synthetic_default_length():
    from lobiflow.trainers.experiment_common import DATASET_CHOICES, DEFAULT_SYNTHETIC_LENGTH
    from lobiflow.trainers.experiments_lobiflow import build_argparser as build_lobiflow_argparser
    from lobiflow.trainers.loss_term_sweep import build_argparser as build_loss_argparser

    assert DEFAULT_SYNTHETIC_LENGTH == 2_000_000
    assert "optiver" in DATASET_CHOICES
    assert build_lobiflow_argparser().parse_args([]).synthetic_length == DEFAULT_SYNTHETIC_LENGTH
    assert build_loss_argparser().parse_args([]).synthetic_length == DEFAULT_SYNTHETIC_LENGTH


def test_context_cli_overrides():
    from lobiflow.trainers.experiment_common import apply_lobiflow_dataset_preset, build_cfg_from_args
    from lobiflow.trainers.benchmark_lobiflow_suite import _aggregate_values
    from lobiflow.trainers.experiments_lobiflow import build_argparser
...
def test_lobiflow_dataset_presets():
    from lobiflow.trainers.experiment_common import (
...
def test_preset_driven_cli_defaults():
    from lobiflow.trainers.experiment_common import apply_lobiflow_dataset_preset
    from lobiflow.trainers.experiments_lobiflow import build_argparser as build_lobiflow_argparser
    from lobiflow.trainers.loss_term_sweep import build_argparser as build_loss_argparser

    run_args = build_lobiflow_argparser().parse_args(["--dataset", "cryptos"])
    run_args = apply_lobiflow_dataset_preset(run_args)
    assert run_args.levels == 10
    assert run_args.history_len == 256
    assert run_args.eval_nfe == 1
    assert run_args.solver == "dpmpp2m"
    assert run_args.ctx_encoder == "hybrid"
    assert run_args.ctx_local_kernel == 7
    assert run_args.ctx_pool_scales == "8,32"
...
def test_resmlp():
    from lobiflow.models.lob_baselines import ResMLP, MLP, build_mlp
    x = torch.randn(4, 32)
    m1 = ResMLP(32, 64, 16)
...
def test_synthetic_dataset():
    from lobiflow.models.lob_baselines import LOBConfig, TransformerFUNet
    from lobiflow.datasets.lob_datasets import build_dataset_splits_synthetic
...
def test_lobster_synth_profile():
    from lobiflow.datasets.lob_datasets import default_lobster_synth_profile_path, load_lobster_synth_profile
...
def test_synthetic_generator_reproducible():
    from lobiflow.datasets.lob_datasets import _generate_synthetic_l2
...
def test_segmented_window_dataset():
    from lobiflow.models.lob_baselines import LOBConfig
    from lobiflow.datasets.lob_datasets import WindowedLOBParamsDataset
...
def test_crypto_month_schedule():
    ...
    from lobiflow.datasets.prepare_cryptos import iter_month_starts
...
def test_crypto_bucket_downsample():
    from lobiflow.utils.lob_utils import keep_last_snapshot_per_bucket
...
def test_databento_day_schedule():
    from lobiflow.datasets.prepare_databento import _iter_day_requests
...
def test_databento_cache_replay_requires_key_only_when_missing():
    ...
    from lobiflow.datasets.prepare_databento import DayRequest, _load_or_download_day
...
def test_databento_default_dataset_path():
    from lobiflow.datasets.lob_datasets import default_es_mbp_10_npz_path

    assert default_es_mbp_10_npz_path().endswith("data\\databento\\es_mbp_10.npz") or default_es_mbp_10_npz_path().endswith("data/databento/es_mbp_10.npz")


def test_optiver_default_dataset_path():
    from lobiflow.datasets.lob_datasets import default_optiver_npz_path

    assert default_optiver_npz_path().endswith("data\\optiver\\optiver_train_8stocks_l2.npz") or default_optiver_npz_path().endswith("data/optiver/optiver_train_8stocks_l2.npz")


def test_databento_segment_downsample_shapes():
    import pandas as pd
    from lobiflow.datasets.prepare_databento import _collect_segment_from_store
...
def test_adaptive_context_resolution():
    from lobiflow.models.lob_baselines import LOBConfig
    from lobiflow.trainers.lob_train_val import resolve_context_length
...
def test_generate_continuation_adaptive_context():
    ...
    from lobiflow.trainers.lob_train_val import generate_continuation
...
def test_training_context_sampling_bounds():
    from lobiflow.models.lob_baselines import LOBConfig
    from lobiflow.trainers.lob_train_val import sample_training_context_length
...
def test_model_name_normalization():
    from lobiflow.trainers.lob_train_val import _normalize_model_name
...
def test_feature_map_roundtrip():
    from lobiflow.datasets.lob_datasets import L2FeatureMap
    L = 5
...
def test_biflow_forward():
    from lobiflow.models.lob_baselines import LOBConfig, BiFlowLOB
...
def test_biflow_nf_forward():
    from lobiflow.models.lob_baselines import LOBConfig, BiFlowNFLOB
...
def test_trades_forward():
    from lobiflow.models.lob_baselines import LOBConfig, DeepMarketTRADESBaseline
...
def test_cgan_forward():
    from lobiflow.models.lob_baselines import LOBConfig, DeepMarketCGANBaseline
...
def test_timecausalvae_forward():
    from lobiflow.models.lob_baselines import LOBConfig, TimeCausalVAEBaseline
...
def test_timegan_forward():
    from lobiflow.models.lob_baselines import LOBConfig, TimeGANBaseline
...
def test_kovae_forward():
    from lobiflow.models.lob_baselines import LOBConfig, KoVAEBaseline
...
def test_structured_regularization_config_overrides():
    ...
    from lobiflow.trainers.experiment_common import build_cfg_from_args
...
def test_regularization_training_curve_smoke(tmp_path):
    ...
    cmd = [
        sys.executable,
        "-m",
        "lobiflow.trainers.regularization_training_curve",
        "--dataset",
        "synthetic",
        "--variants",
    ...
    result = subprocess.run(cmd, cwd=repo_root, check=False, capture_output=True, text=True, timeout=120)
...
def test_train_loop_short():
    from lobiflow.models.lob_baselines import LOBConfig
    from lobiflow.datasets.lob_datasets import build_dataset_splits_synthetic
    from lobiflow.trainers.lob_train_val import train_loop
    cfg = LOBConfig(
...
def test_eval_pipeline():
    from lobiflow.models.lob_baselines import LOBConfig
    from lobiflow.datasets.lob_datasets import build_dataset_splits_synthetic
    from lobiflow.trainers.lob_train_val import train_loop, eval_one_window, eval_many_windows
    cfg = LOBConfig(
        levels=5, history_len=20, hidden_dim=32, batch_size=8,
...
def test_utils():
    from lobiflow.utils.lob_utils import flatten_dict, unflatten_to_nested, microstructure_series
    d = {"a": 1, "b": {"c": 2, "d": {"e": 3}}}
    flat = flatten_dict(d)
